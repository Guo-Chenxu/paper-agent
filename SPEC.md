...
### 对抗Review循环（贯穿全流程，零容忍、零妥协）

**每一自动化阶段必须启动子agent进行对抗式审查，零容忍、零妥协，直至产出完美方可进入下一阶段。**

具体机制如下：

- **循环不设上限**：形成"产出 → 多agent对抗review → 发现问题 → 修正 → 再次对抗review"的闭环，回合数不设上限，**只要还有一个问题未解决就不得退出循环**。
- **审查维度全覆盖**：每轮对抗审查必须覆盖正确性（逻辑正确、事实准确、引用可溯源）、完整性（无遗漏、无跳步、覆盖所有边界情况）、一致性（前后表述一致、图文一致、代码与描述一致），任一维度不达标即刻退回修正。
- **禁止妥协**：严禁"基本合格""影响不大""可以接受"等降级判断，每一次对抗review只有两种结论：**全部通过**或**存在待修复问题**。对模糊地带必须从严判定为问题。
- **子agent独立性**：对抗审查的多个子agent必须独立思考、互不影响，结果通过投票或平均值汇总，分歧过大时（如评分差异≥2分）启动额外agent仲裁。
- **全程记录**：每轮review发现的问题、修正方案、修正后的验证结果均记录在对应阶段的报告中，形成完整审计链。
- **自检闭环**：修正完成后必须再次提交完整对抗review，不得仅检查已标记的问题点，确保未引入新问题。

**关键**：对抗review不是"检查一下"的辅助环节，而是**决定能否进入下一阶段的唯一判据**——通过对抗review的产物才算完成，否则视为该阶段未完成。
