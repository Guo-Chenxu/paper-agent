...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Find ACM-related papers via the OpenAlex compatibility wrapper")
...
def main() -> None:
    ...
    papers = crawler.search(
        query=args.query,
        venue_name="ACM",
        venue_aliases=ACM_ALIASES,
        max_results=args.max_results,
        years=args.years,
    )
    save_json(args.output, papers)
    print(f"ACM papers: {len(papers)} -> {args.output}")
